...
public final class HeuristicManager {
    ...
    public static float[] computePriors(State state, ActionSet actions, HeuristicConfiguration config) {
        ...
        for (int i = 0; i < actions.size(); i++) {
            ...
            if (positionRotationKey != cachedPositionRotation) {
                ...
                cachedTileScore = tilePlacementScore(state, x, y, rotation, config.positionHeuristik());
            }

            scores[i] = computePrior(state, candidateAction, cachedTileScore, config);
        }
