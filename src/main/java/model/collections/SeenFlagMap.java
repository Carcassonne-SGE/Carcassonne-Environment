...
public class SeenFlagMap {
    ...
    /// ensureSeen
    ///
    /// seen is a long array where ach bit is used as flag that can later be set or checked. Is very memory efficient
    /// this function ensures that the array has at least the capacity needed for the given flagsCount may have more
    /// capacity
    ///
    /// ensures the seen aray is empty///
    ...
    /// Ensures that after the call the pos is flaged
    ///
    /// pos is not the long[] array index but the logical index, the bit index
     public boolean notSeenAndSet(int pos) {
        ...
        long mask = 1L << (pos & 63);   // has 1 at the exact pos position (pos & 63) is fast modolu operation to get the lower bits
        long v = seen[word];
        if ((v & mask) != 0L) {
            // the bit was 1 which means it was flaged
            return false;
        }
        seen[word] = v | mask;
